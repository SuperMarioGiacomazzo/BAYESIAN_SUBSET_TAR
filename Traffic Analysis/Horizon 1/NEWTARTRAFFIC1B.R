...
for(day in 1:5){
  ...
  for(series in 1:7){
    ...
    DATA.TRAIN=tar.data.func(day=day,series=series,P=P,h=1,nquant=nquant)
    ...
    while(check<0.90 & modnum<=n.regimes){
    ...
    train.data=tar.data.func(day=day,series=series,P=7,h=1,nquant=50)
    ...
    # png(filename=paste("SELECT1Fit1D",day,"S",series,".png",sep=""),width=800,height=500)
    # plot(train.data$data$y[order.forward1],col="gray50")
    # points(train.predict[order.forward1],type="l")
    # dev.off()
    
    test.data=tar.data.func2(day=day,series=series,P=7,h=1,nquant=50)
    ...
    # png(filename=paste("SELECT1Predict1D",day,"S",series,".png",sep=""),width=800,height=500)
    ...
    cl<-makeCluster(5)
    registerDoParallel(cl)
    while(check2<0.90 & modnum2<=n.vars){
    ...
    # png(filename=paste("SELECT2Fit1D",day,"S",series,".png",sep=""),width=800,height=500)
    # plot(train.data$data$y[order.forward1],col="gray50")
    ...
    # png(filename=paste("SELECT2Predict1D",day,"S",series,".png",sep=""),width=800,height=500)
    # plot(test.data$data$y[order.forward2],col="gray50")
    ...
    save.image("NEWTARTRAFFIC1B.Rdata")
  }
}
