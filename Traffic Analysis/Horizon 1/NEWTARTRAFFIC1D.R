...
for(day in DAY.ID){
  for(series in LOOP.ID){
    ...
    THRESH=seq.thresh.func(day=day,series=series,P=7,h=1,nquant=50)
    REG.IN=rep(NA,51)
...
write.csv(FINAL.MOD.NREGIME,file="NEWFINAL.MOD.NREGIME1.csv")
...
for(day in DAY.ID){
  for(series in LOOP.ID){
    ...
    REG.IN2=REG.IN[-1]
    INIT.DELTA=c(0,rep(NA,(max(FINAL.MOD.NREGIME)-1)))
...
png("NEWThreshold1.png",width=800,height=1000)
ggplot(DELTA) +
...
#Specific Models
EX1=TAR.RESULTS3[[1]][[2]]$mean.coef
EX1
