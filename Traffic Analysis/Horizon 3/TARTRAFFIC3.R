...
for(day in 1:5){
  ...
  TAR.RESULTS[[day]]=foreach(series=1:7)%do%{
    ...
    out=tar.data.func(day=day,series=series,P=7,h=3,nquant=50)
    
    #Initial Seasonal Model Using JAGS
    tarmod=bayesreg(y~.,data=out$data,prior="hs+",nsamples=5000,burnin=5000,thin=1)
    ...
    png(filename=paste("Fit3D",day,"S",series,".png",sep=""),width=800,height=500)
    ...
    png(filename=paste("Predict3D",day,"S",series,".png",sep=""),width=800,height=500)
